/*
 * app_main.c — variant A (manual-esp). ESP32-S3 · custom MQTT control protocol
 * (esp-mqtt) + esp_https_ota self-download. No AWS IoT Jobs. Same
 * boot/commit/rollback sequence as variants B and C.
 */
#include <stdio.h>
...
void app_main(void)
{
    ...
    if (mqtt_es_start(control_protocol_on_publish) != ESP_OK) {
        ESP_LOGW(TAG, "initial MQTT connect did not complete (esp-mqtt keeps retrying)");
    }
    if (trial) self_test_feed_watchdog();

    control_protocol_start();   /* resolves trial boot + runs the custom protocol */

    ESP_LOGI(TAG, "up on v%d.%d.%d (%s) — variant A (custom protocol + esp_https_ota)",
             APP_VERSION_MAJOR, APP_VERSION_MINOR, APP_VERSION_BUILD,
             FW_SELFTEST_SHOULD_PASS ? "vGOOD" : "vBAD");
