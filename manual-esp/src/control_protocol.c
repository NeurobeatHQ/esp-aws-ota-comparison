...
{
    return snprintf(buf, buflen, "dt/%s/ota/%s", THING_NAME, suffix);
}
...
static void publishShouldIUpdate(void)
{
    ...
    int n = snprintf(payload, sizeof(payload), "{\"thing\":\"%s\",\"version\":\"%d.%d.%d\"}",
                     THING_NAME, APP_VERSION_MAJOR, APP_VERSION_MINOR, APP_VERSION_BUILD);
    ESP_LOGI(TAG, "asking server: should-i-update? (v%d.%d.%d)",
             APP_VERSION_MAJOR, APP_VERSION_MINOR, APP_VERSION_BUILD);
    mqtt_es_publish(topic, (uint16_t)tlen, payload, (size_t)n, 1);
}
...
static void subscribeControlTopics(void)
{
    ...
    mqtt_es_subscribe(topic, (uint16_t)tlen, 1);
